package com.takuya.whatanimu.ui.adapter;
...
public class AdapterSearchResults extends RecyclerView.Adapter<AdapterSearchResults.ASRVH> {
    ...
    public void onBindViewHolder(final ASRVH holder, int position) {
        ...
        imageLoader.with(context).placeholder(R.drawable.ic_photo).load(Mapper.getImageUrl(doc)).target(holder.photoIv).callback(new Callback<Boolean>() {
                    @Override
